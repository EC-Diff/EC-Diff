...
class EC_Diff:
    ...
    def __init__(self, cloud, edge, p=6, k=5, a=0.2, s=36):
        self.cloud = cloud
        self.edge = edge
    ...
    def hybridsd(
        ...
        **kwargs,
        ...
        # 0. Default height and width to unet
        height = height or self.cloud.default_sample_size * self.cloud.vae_scale_factor
        width = width or self.cloud.default_sample_size * self.cloud.vae_scale_factor
        ...
        self.cloud._denoising_end = denoising_end
        ...
        self.edge._denoising_end = denoising_end
        ...
        (
            ...
            pooled_prompt_embeds,
            negative_pooled_prompt_embeds,
        ) = self.cloud.encode_prompt(
            prompt=prompt,
            prompt_2=prompt_2,
            device=device,
            num_images_per_prompt=num_images_per_prompt,
            do_classifier_free_guidance=self.cloud.do_classifier_free_guidance,
            negative_prompt=negative_prompt,
            negative_prompt_2=negative_prompt_2,
            prompt_embeds=prompt_embeds,
            negative_prompt_embeds=negative_prompt_embeds,
        ...
        # 7. Prepare added time ids & embeddings
        add_text_embeds = pooled_prompt_embeds
        if self.cloud.text_encoder_2 is None:
            text_encoder_projection_dim = int(pooled_prompt_embeds.shape[-1])
        else:
            text_encoder_projection_dim = self.cloud.text_encoder_2.config.projection_dim

        add_time_ids = self.cloud._get_add_time_ids(
            original_size,
            crops_coords_top_left,
            target_size,
            dtype=prompt_embeds.dtype,
        ...
        if negative_original_size is not None and negative_target_size is not None:
            negative_add_time_ids = self.cloud._get_add_time_ids(
                negative_original_size,
                negative_crops_coords_top_left,
                negative_target_size,
                dtype=prompt_embeds.dtype,
        ...
        # 8.1 Apply denoising_end
        if (
            self.cloud.denoising_end is not None
            and isinstance(self.cloud.denoising_end, float)
            and self.cloud.denoising_end > 0
            and self.cloud.denoising_end < 1
        ):
            discrete_timestep_cutoff = int(
                round(
                    self.cloud.scheduler.config.num_train_timesteps
                    - (self.cloud.denoising_end * self.cloud.scheduler.config.num_train_timesteps)
                )
            )
            num_inference_steps = len(list(filter(lambda ts: ts >= discrete_timestep_cutoff, timesteps)))
            timesteps = timesteps[:num_inference_steps]
            ...
            for i, t in enumerate(timesteps):
                if i<k:
                    ...
                    if self.cloud.do_classifier_free_guidance and self.cloud.guidance_rescale > 0.0:
                        # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                        noise_pred = rescale_noise_cfg(noise_pred, noise_pred_text, guidance_rescale=self.cloud.guidance_rescale)
                    # compute the previous noisy sample x_t -> x_t-1
                    latents_dtype = latents.dtype
                    ...
                    if self.edge.do_classifier_free_guidance and self.edge.guidance_rescale > 0.0:
                        # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
                        noise_pred = rescale_noise_cfg(noise_pred, noise_pred_text, guidance_rescale=self.edge.guidance_rescale)
                    # compute the previous noisy sample x_t -> x_t-1
                    latents_dtype = latents.dtype
        ...
        if not output_type == "latent":
            # make sure the VAE is in float32 mode, as it overflows in float16
            needs_upcasting = self.edge.vae.dtype == torch.float16 and self.edge.vae.config.force_upcast

            if needs_upcasting:
                self.edge.upcast_vae()
                latents = latents.to(next(iter(self.edge.vae.post_quant_conv.parameters())).dtype)
            elif latents.dtype != self.edge.vae.dtype:
                if torch.backends.mps.is_available():
                    # some platforms (eg. apple mps) misbehave due to a pytorch bug: https://github.com/pytorch/pytorch/pull/99272
                    self.edge.vae = self.edge.vae.to(latents.dtype)

            # unscale/denormalize the latents
            # denormalize with the mean and std if available and not None
            has_latents_mean = hasattr(self.edge.vae.config, "latents_mean") and self.edge.vae.config.latents_mean is not None
            has_latents_std = hasattr(self.edge.vae.config, "latents_std") and self.edge.vae.config.latents_std is not None
            if has_latents_mean and has_latents_std:
                latents_mean = (
                    torch.tensor(self.edge.vae.config.latents_mean).view(1, 4, 1, 1).to(latents.device, latents.dtype)
                )
                latents_std = (
                    torch.tensor(self.edge.vae.config.latents_std).view(1, 4, 1, 1).to(latents.device, latents.dtype)
                )
                latents = latents * latents_std / self.edge.vae.config.scaling_factor + latents_mean
            else:
                latents = latents / self.edge.vae.config.scaling_factor

            image = self.edge.vae.decode(latents, return_dict=False)[0]

            # cast back to fp16 if needed
            if needs_upcasting:
                self.edge.vae.to(dtype=torch.float16)
        else:
            image = latents

        if not output_type == "latent":
            # apply watermark if available
            if self.edge.watermark is not None:
                image = self.edge.watermark.apply_watermark(image)

            image = self.edge.image_processor.postprocess(image, output_type=output_type)
